...
def compute_alpha_k(Y_k, y_k, g):
    ...
    for alpha in np.linspace(0, 1, num=100):  # Adjust num for finer resolution
        ...
        convex_combination = g(
            alpha * np.outer(y_k, y_k) + (1 - alpha) * np.dot(Y_k, Y_k.T)
        )
...
def adap_fista(x0, mu, L, phi_s, phi_n, sigma=1, chi=0.5):
    y_i = x_i = x0
    A_i = 0
    tau_i = 1
    i = 0
    ...
    while True:
        ...
        def affine_approx_phi_s(x):
            ...
            # Compute the affine approximation
            affine_approx = function(x_i_tilde) + np.dot(
                grad.flatten(), (x - x_i_tilde).flatten()
            )
        ...
        def q_i(u):
            return (
                affine_approx_phi_s(u)
                + phi_n(u)
                + L_i / 2 * np.linalg.norm(u - x_i_tilde) ** 2
            )
        ...
        result = minimize(
            q_i, initial_guess.flatten(), bounds=bounds
        )  # Minimize the flattened u
...
def adap_aipp(g, lambda_0, rho, W):
    ...
    while j < 10:
        ...
        try:
            ...
            continue
        # ---> check condition <---
        condition = lambda_j * np.linalg.norm(W_j_1) - (
            lambda_j * g(W) + 1 / 2 * np.linalg.norm(W - W_j_1, ord="fro") ** 2
        ) >= np.trace(np.dot(V, W_j_1 - W))
        if not condition:
            lambda_j = lambda_j / 2
            continue
