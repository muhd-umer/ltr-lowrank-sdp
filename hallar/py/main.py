...
print(
    "avg:", max_stable_set, "max:", max(max_stable_sets), "min:", min(max_stable_sets)
)
...
def theta_tilde(Y, p, beta, A, A_adjoint, C, q):
    ...
    min_eigenvalue = np.linalg.eigvalsh(
        C + A_adjoint(q(Y, p, beta, A), n, nodes, edges)
    ).min()
...
def hallar(
    ...
    while True:
        ...
        if AX_b_norm < epsilon_p:  # ord='fro' ?
            print(
                "Stopping criterion met (||A(UU.T)-b|| = {} < {} = epsilon_p)".format(
                    np.round(AX_b_norm, 5), epsilon_p
                )
            )
            break
        ...
        print(
            "Iteration {}: ||A(UU.T)-b|| = {}, L(Y) = {}".format(
                t, np.round(AX_b_norm, 5), np.round(opt_value, 5)
            )
        )
    ...
    return (
        ...
        p_t,
        theta_t,
        L_beta_scipy(optimal_solution_y_flat),
        objective_values,
        constraint_violations,
    )
