...
def gradient_descent(
    L_beta, Y_0, epsilon, p, C, learning_rate=1e-3, max_iterations=1000
):
    Y_k = Y_0
    iteration = 0
...
def plot_metrics(
    graph, beta, rank, objective_values, constraint_violations, reference_value
):
    ...
    ax1.axhline(
        ...
        color="tab:blue",
        linestyle="--",
        label="Reference Value (NetworkX)",
    )
    ...
    plt.savefig(
        "plots/{}_beta{}_rank{}_I{}.jpg".format(graph, beta, rank, iterations[-1]),
        dpi=300,
    )
    plt.show()
