/**
 * @file lorads_alm.c
 * @brief LORADS Augmented Lagrangian Method Implementation
 * @details This file implements the Augmented Lagrangian Method (ALM) for solving
 * semidefinite programming problems. It includes:
...
/**
 * @brief Set gradient for ALM optimization
 * @param ASolver LORADS solver instance
 * @param ACone SDP cone
 * @param R Current iterate matrix
 * @param Grad Output gradient matrix
 * @param iCone Cone index
 * @param rho Penalty parameter
 * @details Computes the gradient of the augmented Lagrangian function with respect to R
 */
...
/**
 * @brief Calculate gradient for ALM optimization
 * @param ASolver LORADS solver instance
 * @param rLpDummy LP variable (unused)
 * @param gradLpDummy LP gradient (unused)
 * @param R Array of R matrices
 * @param Grad Array of gradient matrices
 * @param lagNormSquare Output squared norm of gradient
 * @param rho Penalty parameter
 * @details Computes gradients for all SDP cones and calculates total gradient norm
 */
...
/**
 * @brief Set gradient for LP variables in ALM
 * @param ASolver LORADS solver instance
 * @param lp_cone LP cone
 * @param rLp LP variable
 * @param Grad LP gradient
 * @param iCol Column index
 * @param rho Penalty parameter
 * @details Computes gradient of augmented Lagrangian with respect to LP variables
 */
...
/**
 * @brief Calculate gradient for LP variables in ALM
 * @param ASolver LORADS solver instance
 * @param rLp LP variable
 * @param gradLp LP gradient
 * @param R Array of R matrices
 * @param Grad Array of gradient matrices
 * @param lagNormSquare Output squared norm of gradient
 * @param rho Penalty parameter
 * @details Computes gradients for both LP and SDP variables
 */
...
/**
 * @brief Calculate nth root of a number
 * @param base Base number
 * @param n Root order
 * @return nth root of base
 * @details Handles special cases for negative numbers and even roots:
 * - Returns NAN for negative numbers with even roots
 * - Returns positive root for positive numbers
 * - Returns negative root for negative numbers with odd roots
 */
...
/**
 * @brief Solve cubic equation using Cardano's formula
 * @param a Coefficient of x^3
 * @param b Coefficient of x^2
 * @param c Coefficient of x
 * @param d Constant term
 * @param res Array to store roots
 * @return Number of real roots found (0-3)
 * @details Solves ax^3 + bx^2 + cx + d = 0 using Cardano's formula:
 * - Handles special cases (A=0, B=0)
 * - Computes discriminant and determines root type
...
/**
 * @brief Evaluate quartic function
 ...
 * @details Evaluates ax^4 + bx^3 + cx^2 + dx at point x
 */
...
/**
 * @brief Perform line search for ALM optimization
 * @param rho Penalty parameter
 * @param n Problem dimension
 * @param lambd Dual variables
 * @param p1 First quadratic term
 * @param p2 Second quadratic term
 * @param q0 Linear term
 * @param q1 First gradient term
 * @param q2 Second gradient term
 * @param tau Output step size
 * @return Number of roots found
 * @details Performs line search by solving a cubic equation:
 * - Computes coefficients for cubic equation
 * - Finds optimal step size using root finding
 * - Handles special cases and numerical issues
 * - Returns number of valid roots found
 */
 ...
 * @param ASolver LORADS solver instance
 * @param head L-BFGS history
 * @param gradLpDummy LP gradient (unused)
 * @param dDummy LP direction (unused)
 * @param Grad Array of gradient matrices
 * @param D Array of direction matrices
 * @param innerIter Current inner iteration
 * @details Computes search direction using L-BFGS method for SDP variables
 */
...
/**
 * @brief Compute L-BFGS search direction including LP variables
 * @param params Solver parameters
 * @param ASolver LORADS solver instance
 * @param head L-BFGS history
 * @param gradLp LP gradient
 * @param d LP direction
 * @param Grad Array of gradient matrices
 * @param D Array of direction matrices
 * @param innerIter Current inner iteration
 * @details Computes search direction using L-BFGS method for both SDP and LP variables
 */
...
/**
 * @brief Use gradient as search direction
 * @param ASolver LORADS solver instance
 * @param dDummy LP direction (unused)
 * @param gradLPDummy LP gradient (unused)
 * @param D Array of direction matrices
 * @param Grad Array of gradient matrices
 * @details Uses negative gradient as search direction if inner product is non-negative
 */
...
/**
 * @brief Use gradient as search direction including LP variables
 * @param ASolver LORADS solver instance
 * @param d LP direction
 * @param gradLP LP gradient
 * @param D Array of direction matrices
 * @param Grad Array of gradient matrices
 * @details Uses negative gradient as search direction for both SDP and LP variables
 */
...
/**
 * @brief Compute sum of constraint values for ALM
 * @param ASolver LORADS solver instance
 * @param q1 Output vector
 * @details Computes sum of constraint values across all SDP cones:
 * - Accumulates constraint values from each cone
 * - Uses alpha=1.0 for linear combination
 */
...
/**
 * @brief Compute sum of constraint values for ALM including LP
 * @param ASolver LORADS solver instance
 * @param q1 Output vector
 * @details Computes sum of constraint values across all SDP cones and LP variables:
 * - Accumulates constraint values from LP variables
 * - Accumulates constraint values from each SDP cone
 * - Uses alpha=1.0 for linear combination
 */
...
/**
 * @brief Calculate quadratic terms for ALM
 * @param ASolver LORADS solver instance
 * @param rLpDummy LP variable (unused)
 * @param dDummy LP direction (unused)
 * @param R Array of R matrices
 * @param D Array of direction matrices
 * @param q1 First quadratic term
 * @param q2 Second quadratic term
 * @param p12 Output array for quadratic terms
 * @details Computes quadratic terms needed for line search
 */
...
/**
 * @brief Calculate quadratic terms for ALM including LP
 * @param ASolver LORADS solver instance
 * @param rLp LP variable
 * @param d LP direction
 * @param R Array of R matrices
 ...
 * @param q2 Second quadratic term
 * @param p12 Output array for quadratic terms
 * @details Computes quadratic terms needed for line search, including both SDP and LP variables
 */
...
/**
 * @brief Optimize using ALM with reoptimization
 * @param params Solver parameters
 * @param ASolver LORADS solver instance
 * @param alm_iter_state ALM state
 * @param early_stop Whether to enable early stopping
 * @param rho_update_factor Factor for updating penalty parameter
 * @param timeSolveStart Start time of optimization
 * @return Return code indicating success or failure
 * @details Implements the Augmented Lagrangian Method with reoptimization capabilities:
 * - Handles both SDP and LP variables
 * - Uses L-BFGS for direction computation
 * - Implements dynamic rank adjustment
 * - Supports early stopping based on convergence criteria
 * - Includes adaptive penalty parameter updates
 */
 ...
 * @param timeSolveStart Start time of optimization
 * @return Return code indicating success or failure
 * @details Implements the Augmented Lagrangian Method:
 * - Handles both SDP and LP variables
 * - Uses L-BFGS for direction computation
 * - Implements dynamic rank adjustment
 * - Includes adaptive penalty parameter updates
 * - Supports convergence monitoring and reporting
 */
